<template>
  <Festival24Section :section="{background: 'bsf13orange', layouts: [], style: 'full'}" :extra-padding="false" :padding-bottom="false">
    <div class="border-b border-black hidden lg:block">
      <Container>
        <Space class="md:hidden" />

        <p>
          <Festival24Button to="/festival-2024/film-guide/" :back-arrow="true">
            Back to Films A-Z
          </Festival24Button>
        </p>
        <Space />
    ...
    <div class="bg-bsf13orange rounded-b-3xl relative z-10">
      <Space />
      <Space />
    ...
    <Container>
      ...
      <div class="lg:flex lg:flex-row-reverse space-y-8 lg:space-y-0 justify-between">
        <div id="screenings" class="lg:w-1/3">
          <Festival24Box v-if="$store.state.settings?.festival?.enable_film_guide_screenings" size="sm" class="offset-left lg:min-h-full">
    ...
    <div v-if="post.directors_spotlight">
      <Space />
      <Space />
      ...
      <Container class="text-lg">
        ...
        <Space />
        <Space />
        <Festival24DirectorsSpotlight :bios="post.directors_spotlight" :short="true" />
        <Space />
        <Space />
      </Container>
    </div>
...
export default {
  name: 'Festival24SingleFilm',
  mixins: [eventiveEverywhere, hasFestivalTabs, forceBsf13Layout],
  props: {
    post: {
      type: Object,
      required: true
