  ...
  <div>
    <div v-if="layout.type==='types'">
      ...
      <ul v-if="activeType&&activeType.show_filters&&filters&&filters.length" class="flex flex-wrap -mx-1 my-4">
        ...
        >
          <button
            :class="{'underline': activeFilter&&activeFilter.id===filter.id}"
            @click.prevent="() => { toggleActiveFilter(filter) }"
          >
            {{ filter.name }}
          </button>
        </li>
    ...
    <div class="flex md:-mx-2 lg:-mx-4 flex-wrap">
      <div
        v-for="(bio, key) in bios"
        :key="`bio${bio.id}`"
        class="md:px-2 py-4 lg:p-4 w-full md:w-1/2 lg:w-1/3 xl:w-1/4"
      >
        <Festival24Bio
          :team-member="bio"
          :index="key"
          ...
          :gradient-style="(layout.gradient_style ? layout.gradient_style : null)"
        />
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    setActiveBio (bio) {
      this.teamMember = bio
      this.$store.commit('festival/setActiveBio', { bio: this.teamMember, eventId: this.eventId, bgColour: this.layout.modal_colour })
    }
  }
}
</script>
