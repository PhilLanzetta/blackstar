<template>
  <ClientOnly>
    <div>
      <Festival24ScheduleHeader :post="post" :show-time-view="true" active-view="time" :my-schedule="mySchedule" :scheme="scheme" :clear-filters="clearFilters" />
      <div v-if="ready" class="rounded-t-3xl -mt-5 bg-offWhite -mb-16 pb-16">
        <div v-if="activeDay">
          <div class="relative">
            <Space />
            <Space />
          ...
          <Space />
          <div class="border-y border-[#D6D6D6]">
            <input
              ref="rangeInput"
              v-model="rangeScroll"
              type="range"
          ...
          >
            ...
            <div
              ...
              :key="`venue${venue.id}`"
              class="border-b border-[#D6D6D6] w-[3600px] relative select-none odd:bg-[#D6D6D6]"
...
export default {
  name: 'Festival25ScheduleByTime',
  mixins: [hasFestivalSchedule, forceBsf14Layout],
  beforeRouteLeave () {
...
.timetable__grid {
  @apply absolute top-10 bottom-0 left-0 flex border-l border-[#D6D6D6] w-full z-0;
}
...
.timetable__grid > div > span {
  @apply border-r border-[#D6D6D6] uppercase text-sm p-1 flex w-full h-full items-start text-center justify-center relative;
}
...
.timetable__grid--header > div {
  @apply bg-[#D6D6D6] sticky top-[200px];
}

.timetable__grid--header > div > span {
  @apply py-5 border-y border-[#D6D6D6] text-sm;
}
...
input[type=range]::-webkit-slider-thumb {
  ...
  @apply cursor-grab bg-[#D6D6D6] rounded-3xl;
}
