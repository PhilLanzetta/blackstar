<template>
  <div>
    <ScrollMarquee symbol="arrows-down" word2="News" word1="Press" force-colour="black" />
    <Container>
      ...
      <div class="flex justify-center py-20 text-black">
        <site-link v-if="totalPages>currentPage" v-cursor-interact :to="`/news/${(activeFilter ? `${(activeFilter.taxonomy==='post-year' ? 'year/' : '')}${(activeFilter.taxonomy==='post_tag' ? 'tagged/' : '')}${activeFilter.slug}/` : '')}page/${currentPage+1}`">
...
export default {
  ...
  async fetch () {
    this.filters = await this.$axios.$get(`${this.$axiosConfig.cmsUrl}/wp-json/wp/v2/categories?routes=1`)
  }
}
</script>
