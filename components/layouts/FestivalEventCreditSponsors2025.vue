<template>
  <div v-if="additionalCredits">
    <Space />
    <div v-for="(additionalCredit, key) in additionalCredits" :key="`additionalCredit${key}`"
         :class="{'mb-8': key+1 < additionalCredits.length, 'flex flex-wrap items-center justify-start': $store.state.theme==='festival'}">
      <h3
        :class="{'text-xl mr-6': $store.state.theme==='festival', 'text-3xl font-medium mb-4': $store.state.theme==='bsf11', 'text-2xl mb-8 font-normal': $store.state.theme==='bsf12'}">
      ...
      <div v-if="additionalCredit.type==='manual_multiple'" class="flex flex-wrap items-center justify-start">
          <span v-for="(manualCredit, manualCreditKey) in additionalCredit.manual_credits"
                :key="`manualCredit${manualCreditKey}`" class="flex flex-wrap items-center mr-4">
            ...
            <span
              v-if="additionalCredit.manual_credits.length>1&&(manualCreditKey+1)<additionalCredit.manual_credits.length"
              :class="{'font-festival_sans_light_italic': $store.state.theme==='festival'}"><span
              v-if="manualCreditKey+2<additionalCredit.manual_credits.length">,</span><span v-else
...
<script>
export default {
  name: 'FestivalEventCreditSponsors2025',
  props: {
    additionalCredits: {
      type: Array,
      required: true
