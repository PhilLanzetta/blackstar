<template>
  <div class="min-h-screen">
    <Festival24ScheduleHeader :post="post" :show-time-view="showTimeView" :active-view="activeView" :my-schedule="mySchedule" :scheme="scheme" :clear-filters="clearFilters" :year="2025" />
    <div class="rounded-t-3xl -mt-5 " :class="{'bg-offWhite': activeView!=='day'&&scheme!=='orange', 'bg-bsf14orangeTint': activeView!=='day'&&scheme==='orange',  'bg-offWhite': activeView!=='day'&&scheme==='red', 'bg-bsf14pink': activeView==='day'&&!scheme, 'bg-bsf14cherryTint': activeView==='day'&&scheme==='red'}" v-if="ready">
      <Space />
      ...
      <template v-if="activeView==='day'&&!filtered&&activeDay">
        ...
        <Space class="hidden md:block" />
        <div class="border-t-4 border-dotted border-black w-full">
          <Space class="hidden md:block" />
          <Space />
          <Space />
        </div>
...
export default {
  name: 'Festival25Schedule',
  mixins: [hasFestivalSchedule, updateRouteQuery, forceBsf14Layout],
  props: {
    post: {
      type: Object,
      required: true
