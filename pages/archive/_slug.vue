...
export default {
  ...
  async asyncData ({ $axiosConfig, $axios, params, payload, error, store }) {
    ...
    const posts = await $axios.$get(`${$axiosConfig.cmsUrl}/wp-json/wp/v2/pages?slug=${params.slug}&parent_slug=archive&per_page=1`, $axiosConfig.config)
  ...
  data () {
    return {
      endpoint: 'pages',
      endpointArgs: '&parent_slug=archive'
    }
  }
}
</script>
