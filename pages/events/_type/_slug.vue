...
export default {
  ...
  async asyncData ({ $axiosConfig, $axios, params, payload, error, store }) {
    ...
    const posts = await $axios.$get(`${$axiosConfig.cmsUrl}/wp-json/wp/v2/program-event?slug=${params.slug}&parent=0&per_page=1&program-type_name=${params.type}`, $axiosConfig.config)
