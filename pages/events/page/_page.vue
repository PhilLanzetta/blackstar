<template>
  <EventsArchive :posts="posts" :total-pages="totalPages" :current-page="currentPage" />
</template>
<script>
export default {
  ...
  async asyncData ({ $axiosConfig, $axios, params, error, payload, store }) {
    ...
    try {
      const postsResponse = await $axios.get(`${$axiosConfig.cmsUrl}/wp-json/wp/v2/program-event?per_page=6&page=${params.page}`, $axiosConfig.config)
