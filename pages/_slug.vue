...
export default {
  ...
  layout ({ params }) {
    if (params.slug && (params.slug.includes('seen/') || params.slug === 'seen')) {
      return 'seen'
    }
  ...
  async asyncData ({ $axiosConfig, $axios, params, payload, error, store }) {
    ...
    const posts = await $axios.$get(`${$axiosConfig.cmsUrl}/wp-json/wp/v2/pages?slug=${params.slug}&parent=0&per_page=1`, $axiosConfig.config)
